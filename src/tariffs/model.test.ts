...
describe('electricity tariff', () => {
  it('calculates only a flat energy estimate from daily kWh without rounding the input', () => {
    ...
    const tou = plan()
    tou.rates[32][0] = 0.6
    expect(estimateDailyCost(tou, 10)).toBeNull()
  ...
  it('uses tariff time zone and half-hour boundaries across DST, including Sunday', () => {
    ...
    tou.rates[3][6] = 0.6
    tou.rates[4][6] = 0.8
    expect(currentRate(tou, new Date('2026-11-01T08:30:00Z'))).toBe(0.6)
    expect(currentRate(tou, new Date('2026-11-01T09:30:00Z'))).toBe(0.6)
    expect(currentRate(tou, new Date('2026-11-01T10:00:00Z'))).toBe(0.8)
  })
